...
if [[ -f /tmp/test_ops_check.log ]]; then
    ...
    log_test "系统巡检模块可运行" "FAIL: 输出文件未生成"
fi
...
if [[ -f "${sample_log}" ]]; then
    ...
    if [[ -f /tmp/test_ops_log.log ]]; then
        log_test "日志分析模块可运行" "PASS"
    else
        log_test "日志分析模块可运行" "FAIL: 输出文件未生成"
    fi
else
...
if [[ -f /tmp/test_ops_check.log ]]; then
    ...
    if [[ -f /tmp/test_ops_diag.md ]]; then
        log_test "诊断建议模块可运行" "PASS"
    else
        log_test "诊断建议模块可运行" "FAIL: 输出文件未生成"
    fi
else
...
if [[ -f /tmp/test_ops_check.log && -f /tmp/test_ops_log.log && -f /tmp/test_ops_diag.md ]]; then
    ...
    if [[ -f /tmp/test_ops_report.md ]]; then
        log_test "报告生成模块可运行" "PASS"
    else
        log_test "报告生成模块可运行" "FAIL: 输出文件未生成"
    fi
else
